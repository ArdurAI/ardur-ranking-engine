...
export function validateAggregationArtifact(raw: unknown): AggregationArtifact {
  ...
  if (factsByCluster !== null && factsByCluster !== undefined && typeof factsByCluster === 'object' && !Array.isArray(factsByCluster)) {
    for (const [clusterId, facts] of Object.entries(factsByCluster as Record<string, unknown>)) {
      if (!Array.isArray(facts)) continue;
      for (let i = 0; i < facts.length; i++) {
        const result = ExtractedFactSchema.safeParse(facts[i]);
        if (!result.success) {
          throw new Error(
            `Invalid ExtractedFact at factsByCluster["${clusterId}"][${i}]: ${result.error.message}`,
          );
        }
      }
    }
