...
import { SCHEMA_VERSION, CONTRACT_REVISION } from "./contracts.js";
...
function readInput(path) {
    ...
    try {
        ...
        return emitError('IO_READ', `cannot read input: ${path}`, err instanceof Error ? err.message : String(err));
    }
    let parsed;
    try {
        ...
        return emitError('INPUT_PARSE', 'input is not valid JSON', err instanceof Error ? err.message : String(err));
    }
    try {
        return validateAggregationArtifact(parsed);
    }
    catch (err) {
        return emitError('INPUT_INVALID', 'input failed contract validation', err instanceof Error ? err.message : String(err));
    }
}
function writeOutput(path, data) {
    ...
    try {
        ...
        emitError('IO_WRITE', `cannot write output: ${path}`, err instanceof Error ? err.message : String(err));
    }
}
// ---------------------------------------------------------------------------
// Main
...
function main() {
    ...
    if (flags.provider !== 'deterministic') {
        process.stderr.write(`ardur-ranking-engine: warning: --provider ${flags.provider} ignored; ranking is always deterministic\n`);
    }
    ...
    try {
        ...
        emitError('RANKING_FAILED', 'unexpected error during ranking', err instanceof Error ? err.message : String(err));
    }
