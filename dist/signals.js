...
export function corroborationScore(ownerCount, profile) {
...
export function tierValue(tier, profile) {
    ...
    const rankMap = profile.sourceTier.rankByTaxonomy;
...
export function sourceTierBlend(maxTier, meanTier, profile) {
...
export function recencyHalfLifeHours(technicalSignificance, profile) {
...
export function recencyDecay(ageHours, halfLifeHours) {
...
export function diversityMultiplier(normalizedDiversity, profile) {
...
export function engagementScore(velocityByPlatform, profile) {
    const entries = Object.entries(velocityByPlatform);
    if (entries.length === 0)
        return 0; // no data → other signals carry the topic
    let sum = 0;
    let count = 0;
...
export function ageHoursSince(isoTimestamp, now) {
 ...
 */
export function sourceTierSignal(input) {
    ...
    for (const [tier, rawCount] of Object.entries(cluster.tierHistogram)) {
        ...
        if (count <= 0)
            continue;
        const value = tierValue(tier, profile);
...
export function hasTier1Primary(cluster, profile) {
...
export function maxTierValue(cluster, profile) {
    let max = 0;
    for (const [tier, count] of Object.entries(cluster.tierHistogram)) {
        if ((count ?? 0) > 0) {
            max = Math.max(max, tierValue(tier, profile));
        }
    }
    return max;
}
...
const AI_PLATFORM_PATTERN = /\b(generative[\s-]+ai|large[\s-]+language[\s-]+model|\bllm\b|foundation[\s-]+model|kubernetes|\bk8s\b|cloud[\s-]+native|platform[\s-]+engineering|opentelemetry|\bebpf\b|\bgpu\b|\bcuda\b|machine[\s-]+learning[\s-]+(?:model|framework)|deep[\s-]+learning|neural[\s-]+network|diffusion[\s-]+model|inference[\s-]+engine|fine[\s-]?tun)\b/i;
/** Build a lowercase search corpus from the cluster headline + all member texts. */
function buildCorpus(input) {
    const parts = [input.cluster.headline, input.cluster.topic, input.cluster.topicLabel];
    for (const m of input.members) {
        parts.push(m.title);
        if (m.summaryHint)
            parts.push(m.summaryHint);
    }
    return parts.join(' ').toLowerCase();
}
function sourceCategory(tier) {
    if (tier === 'primary')
        return 'official';
    if (tier === 'paper')
        return 'academic';
 ...
 */
export function normalizeOwnerDomain(domain) {
 ...
 */
export function countIndependentOwners(input) {
 ...
 */
export function technicalSignificanceSignal(input) {
 ...
 */
export function ownerDiversity(input) {
    const { members } = input;
    if (members.length === 0) {
    ...
    if (domainCounts.size <= 1)
        return 0; // Single owner → echo penalty
 ...
 */
export function platformVelocities(input) {
    const { members } = input;
    let totalVelocity = 0;
 ...
 */
export function factCorroborationSignal(facts, profile) {
    if (facts.length === 0)
        return 0;
    let sum = 0;
 ...
 */
export function corroborationSignal(input) {
