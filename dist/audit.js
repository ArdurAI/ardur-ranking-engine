 ...
 */
import { toScoreBreakdown } from "./score.js";
/** Deterministic 32-bit FNV-1a hash → unsigned int (stable across runs/machines). */
export function stableHashNumber(input) {
    let hash = 0x811c9dc5;
...
export function auditIdFor(input) {
...
export function explainRanking(input) {
    ...
    return (`${input.gate} (confidence ${pct(input.confidence)}): ` +
